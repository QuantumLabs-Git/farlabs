...
This worker process pulls queued inference tasks from Redis, runs real AI inference,
...
USE_REAL_INFERENCE = os.getenv("USE_REAL_INFERENCE", "true").lower() in {"1", "true", "yes"}

# Model cache
MODEL_CACHE: Dict[str, tuple[Any, Any]] = {}  # model_id -> (model, tokenizer)
...
async def run_real_inference(task: Dict[str, Any]) -> Dict[str, Any]:
    """Run actual AI inference using transformers."""
    prompt = task.get("prompt", "")
    model_id = task.get("model", "gpt2")
    max_tokens = task.get("max_tokens", 100)
    temperature = task.get("temperature", 0.7)

    print(f"Running inference: model={model_id}, max_tokens={max_tokens}")
    ...
    try:
        ...
        tokens_per_second = int(tokens_generated / elapsed) if elapsed > 0 else 0

        print(f"✓ Generated {tokens_generated} tokens in {elapsed:.1f}s ({tokens_per_second} tok/s)")
...
async def simulate_inference(task: Dict[str, Any]) -> Dict[str, Any]:
    """Generate a synthetic response (fallback for testing without GPU)."""
    prompt = task.get("prompt", "")
    ...
    generated_text = (
        ...
        "To enable real AI inference, set USE_REAL_INFERENCE=true and ensure "
        "the worker has access to a GPU or CPU with sufficient resources."
    )

    import random
    tokens_generated = min(int(max_tokens * random.uniform(0.4, 0.7)), 2048)
...
async def handle_task(client: redis.Redis, task: Dict[str, Any]) -> None:
    ...
    if USE_REAL_INFERENCE:
        await publish_progress(
            client,
            task_id,
            {"status": "running", "message": "Loading model and generating...", "progress": 0.3},
        )
        result = await run_real_inference(task)
    else:
        await asyncio.sleep(0.5)
        await publish_progress(
            client,
            task_id,
            {"status": "running", "message": "Generating tokens (simulated)", "progress": 0.5},
        )
        await asyncio.sleep(0.5)
